...
def ensure_unique_session():
    ...
    # Assegna il logger alla sessione 
...
def evaluate():
    ...
    logger = session['logger']
    logger.info(f"{timestamp}###{algorithm_name}###" + "###".join(str(value) for value in session['user_info'].values())  + "###" + ";;;".join(map(str, movie_ids)) + "###" + ";;;".join(selected_titles)  )
...
def reload_embeddings():
    if 'user_profile' in session and 'movies' in session['user_profile']:
        ...
        # Se ci sono film nella sessione, ricarica gli embedding
        if movies:
            user_profile['graph_embeddings'] = []
            user_profile['sbert_embeddings'] = []
            user_profile['compgcn_embeddings'] = []
...
def submit_info():
    ...
    # Memorizziamo il timestamp sarà uguale per tutta la sessione
